from .files_utils import get_selected_files_paths, get_selected_files_names
import pandas as pd
import re
import fmgen.utils.data_manipulation as dm

def _initialize_fm_dataframe(path:any = None):
...
def _normalize_values(df:pd.DataFrame) -> pd.DataFrame:
    ...
    if 'Salário' in df.columns:
        df['Salário'] = df['Salário'].apply(dm.fm_normalize_wage_values)
        
    if 'Mins' in df.columns:    
        df['Mins'] = df['Mins'].apply(dm.fm_normalize_minutes_values)
    
    if 'Valor Estimado' in df.columns:
        df['Valor Estimado'] = df['Valor Estimado'].apply(dm.fm_normalize_estimated_values)
        df = dm.fm_create_max_min_estimated_column(df)
        df = df.drop(columns='Valor Estimado', errors='ignore')
...
def _replace_hyphen_with_zero(df:pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.replace('-', 0)
    df['Personalidade'] = df['Personalidade'].replace(0,'-')
...
def _rename_columns_names(df:pd.DataFrame) -> pd.DataFrame:
    ...
    'Dft': 'defesas_dificeis', 'Ds':'defesas_seguras',
